...
_, train_ds = ellipse_fan_radon_astra(im_shape=im_shape,
                                      im_size=im_size,
                                      D_source=D_source,
                                      D_det=D_det,
                                      dist_det=dist_det,
...
_, val_ds = ellipse_fan_radon_astra(im_shape=im_shape,
                                    im_size=im_size,
                                    D_source=D_source,
                                    D_det=D_det,
                                    dist_det=dist_det,
...
net = ImgLieResnet(num_targets=2,**{'k':128,'total_ds':.1,'fill':.05,'nbhd':25,'group':SE2(.05),'liftsamples':4})
...
class InvariantModel(LightningModule):
    ...
    def on_validation_epoch_end(self):
        if self.validation_summed_error < self.best_validation_summed_error:
            torch.save(net.state_dict(), f'ellipse_lieconv_reco_{dataset_size}_best.pt')
            self.best_validation_summed_error = self.validation_summed_error
        self.validation_summed_error = 0.


    def configure_optimizers(self):
        optim = torch.optim.AdamW(self.parameters(), lr=2e-3, weight_decay=0.01)
...
trainer = Trainer(
    logger=TensorBoardLogger(save_dir='tmp_logs/', name=f'ellipse_lieconv_reco_{dataset_size}'),
    gpus=AVAIL_GPUS,
    max_epochs=3000,
...
torch.save(net.state_dict(), f'ellipse_lieconv_reco_{dataset_size}.pt')