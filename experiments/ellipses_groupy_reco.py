...
from adambielsky import ResNet, BasicBlock
net = ResNet(BasicBlock, [2,2,2,2], num_classes=2, out_size=2896)
...
class InvariantModel(LightningModule):
    ...
    def on_validation_epoch_end(self):
        if self.validation_summed_error < self.best_validation_summed_error:
            torch.save(net.state_dict(), f'ellipse_groupy_reco_{dataset_size}_best.pt')
...
trainer = Trainer(
    logger=TensorBoardLogger(save_dir='tmp_logs/', name=f'ellipse_groupy_reco_{dataset_size}'),
    gpus=AVAIL_GPUS,
    max_epochs=3000,
...
torch.save(net.state_dict(), f'ellipse_groupy_reco_{dataset_size}.pt')