...
from adambielsky import ResNet18
...
class InvariantModel(LightningModule):
    ...
    def on_validation_epoch_end(self):
        if self.validation_summed_error < self.best_validation_summed_error:
            torch.save(net.state_dict(), f'{save_path}/rotmnist_affine_groupy_discrete_reco_{n_angles}_best.pt')
...
trainer = Trainer(
    logger=TensorBoardLogger(save_dir='tmp_logs/', name=f'{save_path}/rotmnist_affine_groupy_discrete_reco_{n_angles}'),
    gpus=AVAIL_GPUS,
    max_epochs=400,
...
torch.save(net.state_dict(), f'{save_path}/rotmnist_affine_groupy_discrete_reco_{n_angles}.pt')