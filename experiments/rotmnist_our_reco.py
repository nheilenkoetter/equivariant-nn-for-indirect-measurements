...
class Reshape:
    def __call__(self,image):
        #print(image.shape)
        return image.reshape(29*29, 1)

ds = MnistRotDataset(root='/home/nick7/equivariance/equivariant-nn-for-indirect-measurements/experiments/data/', train=True, transform=transforms.Compose([ZeroPad(), reco, Reshape()]))
...
inp_points = torch.stack(torch.meshgrid(torch.linspace(-r, r, 29), torch.linspace(-r, r, 29)), dim=-1)
inp_points = inp_points.reshape(29*29,2)
print('inp', inp_points.shape)

net = SE2InvariantNet(SE2onR2,
                      radius=r,
...
class InvariantModel(LightningModule):
    ...
    def on_validation_epoch_end(self):
        if self.validation_summed_error < self.best_validation_summed_error:
            torch.save(net.state_dict(), f'{save_path}/rotmnist_our_reco_{n_angles}_best.pt')
...
trainer = Trainer(
    logger=TensorBoardLogger(save_dir='tmp_logs/', name=f'{save_path}/rotmnist_our_reco_{n_angles}'),
    gpus=AVAIL_GPUS,
    max_epochs=400,
...
torch.save(net.state_dict(), f'{save_path}/rotmnist_our_reco_{n_angles}.pt')